...
s1 = sprintf("Annual KOR NGDP Growth = %2.2f", kor_ngdp_ag);
disp(s1)

kor_rgdp_qg = meanc(kor_rgdp_growth);
kor_rgdp_ag = 4*kor_rgdp_qg;
s2 = sprintf("Annual KOR RGDP Growth = %2.2f", kor_rgdp_ag);
disp(s2)

kor_gdpdef_qg = meanc(kor_gdpdef_growth);
kor_gdpdef_ag = 4*kor_gdpdef_qg;
s3 = sprintf("Annual KOR GDPDEF Growth = %2.2f", kor_gdpdef_ag);
disp(s3)
